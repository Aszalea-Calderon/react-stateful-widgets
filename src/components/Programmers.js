...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    // It's going to utilize both slices of state to return the _name_ of the featured data.
  ...
  const style = {
    fontSize: "1.5em",
    marginTop: "0.5em",
    color: name ? "gold" : "royalblue", // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className="programmers">
        {
          ...
          list.map((data) => (
            <div className="programmer" key={data.id}>
              {data.name}{" "}
              <button
                onClick={() => {
                  setName(data.name);
                  /* in here set the featured id to be data.id */
                }}
              >
                Feature
              </button>
