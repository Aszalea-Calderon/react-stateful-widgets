...
export default function Squares() {
  ...
  const getClassName = (id) => {
    ...
    // It should return a string containing the class name of 'active', if the id passed --- if this true
    ...
    if (id === squares) {
      //If id is equal to the square coming in, make the className active, otherwise nothing
      return "active";
    } else {
      return "";
    }
  ...
  const markActive = (id) => {
    ...
    // Set the id argument to become the active id in state.. when things come in , set the id to be active. otheresie reset. id
